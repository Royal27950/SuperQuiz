...
public class QuizDbHelperDeadpool extends SQLiteOpenHelper {

    // Database name
    public static String DATABASE_QUESTION = "questionBankDeadpool.db";
    ...
    public QuizDbHelperDeadpool(Context context) {
     ...
     */
    public long addInitialQuestionDeadpool (DeadpoolQuestion question) {
        SQLiteDatabase deadpoolDb = this.getWritableDatabase();
        ...
        long insert = deadpoolDb.insert(TABLE_QUESTION, null, values);
        return insert;
    }
     ...
     * Question
     */
    public List<DeadpoolQuestion> getAllQuestionsListDeadpool() {
        List<DeadpoolQuestion> questionArrayList = new ArrayList<>();
        String selectQuery = "SELECT  * FROM " + TABLE_QUESTION;

        SQLiteDatabase deadpoolDb = this.getReadableDatabase();
        Cursor c = deadpoolDb.rawQuery(selectQuery, null);

        // looping through all records and adding to the list
        if (c.moveToFirst()) {
            do {
                DeadpoolQuestion question = new DeadpoolQuestion();
