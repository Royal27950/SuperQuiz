...
public class QuizDbHelperOnePiece extends SQLiteOpenHelper {

    // Database name
    public static String DATABASE_QUESTION = "questionBankOnePiece.db";
    // Current version of database
    private static final int DATABASE_VERSION = 3;
    // Database table name
    private static final String TABLE_QUESTION = "QuestionBankOnePiece";
    ...
    public QuizDbHelperOnePiece(Context context) {
     ...
     */
    public long addInitialQuestionOnePiece (OnePieceQuestion question) {
        SQLiteDatabase onePieceDb = this.getWritableDatabase();
        ...
        long insert = onePieceDb.insert(TABLE_QUESTION, null, values);
        return insert;
    }
     ...
     * Question
     */
    public List<OnePieceQuestion> getAllQuestionsListOnePiece() {
        List<OnePieceQuestion> questionArrayList = new ArrayList<>();
        String selectQuery = "SELECT  * FROM " + TABLE_QUESTION;

        SQLiteDatabase onePieceDb = this.getReadableDatabase();
        Cursor c = onePieceDb.rawQuery(selectQuery, null);

        // looping through all records and adding to the list
        if (c.moveToFirst()) {
            do {
                OnePieceQuestion question = new OnePieceQuestion();
