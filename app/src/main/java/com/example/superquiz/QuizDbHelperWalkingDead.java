...
public class QuizDbHelperWalkingDead extends SQLiteOpenHelper {
    // Database name
    public static String DATABASE_QUESTION = "questionBankWalkingDead.db";
    // Current version of database
    private static final int DATABASE_VERSION = 3;
    // Database table name
    private static final String TABLE_QUESTION = "QuestionBankWalkingDead";
    ...
    public QuizDbHelperWalkingDead(Context context) {
     ...
     */
    public long addInitialQuestionWalkingDead (WalkingDeadQuestion question) {
        SQLiteDatabase walkingDeadDb = this.getWritableDatabase();
        ...
        long insert = walkingDeadDb.insert(TABLE_QUESTION, null, values);
        return insert;
    }
     ...
     * Question
     */
    public List<WalkingDeadQuestion> getAllQuestionsListWalkingDead() {
        List<WalkingDeadQuestion> questionArrayList = new ArrayList<>();
        String selectQuery = "SELECT  * FROM " + TABLE_QUESTION;

        SQLiteDatabase walkingDeadDb = this.getReadableDatabase();
        Cursor c = walkingDeadDb.rawQuery(selectQuery, null);

        // looping through all records and adding to the list
        if (c.moveToFirst()) {
            do {
                WalkingDeadQuestion question = new WalkingDeadQuestion();
