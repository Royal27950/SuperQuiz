...
public class QuizDbHelperMyHeroAcademia extends SQLiteOpenHelper {

    // Database name
    public static String DATABASE_QUESTION = "questionBankMyHeroAcademia.db";
    // Current version of database
    private static final int DATABASE_VERSION = 3;
    // Database table name
    private static final String TABLE_QUESTION = "QuestionBankMyHeroAcademia";
    ...
    public QuizDbHelperMyHeroAcademia(Context context) {
     ...
     */
    public long addInitialQuestionMyHeroAcademia (MyHeroAcademiaQuestion question) {
        SQLiteDatabase myHeroAcademiaDb = this.getWritableDatabase();
        ...
        long insert = myHeroAcademiaDb.insert(TABLE_QUESTION, null, values);
        return insert;
    }
     ...
     * Question
     */
    public List<MyHeroAcademiaQuestion> getAllQuestionsListMyHeroAcademia() {
        List<MyHeroAcademiaQuestion> questionArrayList = new ArrayList<>();
        String selectQuery = "SELECT  * FROM " + TABLE_QUESTION;

        SQLiteDatabase myHeroAcademiaDb = this.getReadableDatabase();
        Cursor c = myHeroAcademiaDb.rawQuery(selectQuery, null);

        // looping through all records and adding to the list
        if (c.moveToFirst()) {
            do {
                MyHeroAcademiaQuestion question = new MyHeroAcademiaQuestion();
