...
public class QuizDbHelperTokyoGhoul extends SQLiteOpenHelper {
    // Database name
    public static String DATABASE_QUESTION = "questionBankTokyoGhoul.db";
    // Current version of database
    private static final int DATABASE_VERSION = 3;
    // Database table name
    private static final String TABLE_QUESTION = "QuestionBankTokyoGhoul";
    ...
    public QuizDbHelperTokyoGhoul(Context context) {
     ...
     */
    public long addInitialQuestionTokyoGhoul (TokyoGhoulQuestion question) {
        SQLiteDatabase tokyoGhoul = this.getWritableDatabase();
        ...
        long insert = tokyoGhoul.insert(TABLE_QUESTION, null, values);
        return insert;
    }
     ...
     * Question
     */
    public List<TokyoGhoulQuestion> getAllQuestionsListTokyoGhoul() {
        List<TokyoGhoulQuestion> questionArrayList = new ArrayList<>();
        String selectQuery = "SELECT  * FROM " + TABLE_QUESTION;

        SQLiteDatabase tokyoGhoul = this.getReadableDatabase();
        Cursor c = tokyoGhoul.rawQuery(selectQuery, null);

        // looping through all records and adding to the list
        if (c.moveToFirst()) {
            do {
                TokyoGhoulQuestion question = new TokyoGhoulQuestion();
