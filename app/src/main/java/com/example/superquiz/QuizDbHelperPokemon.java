...
public class QuizDbHelperPokemon extends SQLiteOpenHelper{
    // Database name
    public static String DATABASE_QUESTION = "questionBankPokemon.db";
    // Current version of database
    private static final int DATABASE_VERSION = 3;
    // Database table name
    private static final String TABLE_QUESTION = "QuestionBankPokemon";
    ...
    public QuizDbHelperPokemon(Context context) {
     ...
     */
    public long addInitialQuestionPokemon (PokemonQuestion question) {
        SQLiteDatabase pokemonDb = this.getWritableDatabase();
        ...
        long insert = pokemonDb.insert(TABLE_QUESTION, null, values);
        return insert;
    }
     ...
     * Question
     */
    public List<PokemonQuestion> getAllQuestionsListPokemon() {
        List<PokemonQuestion> questionArrayList = new ArrayList<>();
        String selectQuery = "SELECT  * FROM " + TABLE_QUESTION;

        SQLiteDatabase pokemonDb = this.getReadableDatabase();
        Cursor c = pokemonDb.rawQuery(selectQuery, null);

        // looping through all records and adding to the list
        if (c.moveToFirst()) {
            do {
                PokemonQuestion question = new PokemonQuestion();
