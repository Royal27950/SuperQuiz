...
public class QuizDbHelperGeography extends SQLiteOpenHelper {

    // Database name
    public static String DATABASE_QUESTION = "questionBankGeography.db";
    // Current version of database
    private static final int DATABASE_VERSION = 3;
    // Database table name
    private static final String TABLE_QUESTION = "QuestionBankGeography";
    ...
    public QuizDbHelperGeography(Context context) {
     ...
     */
    public long addInitialQuestionGeography (GeographyQuestion question) {
        SQLiteDatabase geographyDb = this.getWritableDatabase();
        ...
        long insert = geographyDb.insert(TABLE_QUESTION, null, values);
        return insert;
    }
     ...
     * Question
     */
    public List<GeographyQuestion> getAllQuestionsListGeography() {
        List<GeographyQuestion> questionArrayList = new ArrayList<>();
        String selectQuery = "SELECT  * FROM " + TABLE_QUESTION;

        SQLiteDatabase geographyDb = this.getReadableDatabase();
        Cursor c = geographyDb.rawQuery(selectQuery, null);

        // looping through all records and adding to the list
        if (c.moveToFirst()) {
            do {
                GeographyQuestion question = new GeographyQuestion();
