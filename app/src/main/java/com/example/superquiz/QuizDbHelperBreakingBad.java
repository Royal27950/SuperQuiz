...
public class QuizDbHelperBreakingBad extends SQLiteOpenHelper {

    // Database name
    public static String DATABASE_QUESTION = "questionBankBreakingBad.db";
    // Current version of database
    private static final int DATABASE_VERSION = 4;
    // Database table name
    private static final String TABLE_QUESTION = "QuestionBankBreakingBad";
    ...
    public QuizDbHelperBreakingBad(Context context) {
     ...
     */
    public long addInitialQuestionBreakingBad (BreakingBadQuestion question) {
        SQLiteDatabase breakingBadDb = this.getWritableDatabase();
        ...
        long insert = breakingBadDb.insert(TABLE_QUESTION, null, values);
        return insert;
    }
     ...
     * Question
     */
    public List<BreakingBadQuestion> getAllQuestionsListBreakingBad() {
        List<BreakingBadQuestion> questionArrayList = new ArrayList<>();
        String selectQuery = "SELECT  * FROM " + TABLE_QUESTION;

        SQLiteDatabase breakingBadDb = this.getReadableDatabase();
        Cursor c = breakingBadDb.rawQuery(selectQuery, null);

        // looping through all records and adding to the list
        if (c.moveToFirst()) {
            do {
                BreakingBadQuestion question = new BreakingBadQuestion();
