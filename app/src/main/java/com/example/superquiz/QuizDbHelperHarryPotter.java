...
public class QuizDbHelperHarryPotter extends SQLiteOpenHelper {

    // Database name
    public static String DATABASE_QUESTION = "questionBankHarryPotter.db";
    // Current version of database
    private static final int DATABASE_VERSION = 3;
    // Database table name
    private static final String TABLE_QUESTION = "QuestionBankHarryPotter";
    ...
    public QuizDbHelperHarryPotter(Context context) {
     ...
     */
    public long addInitialQuestionHarryPotter (HarryPotterQuestion question) {
        SQLiteDatabase harryPotterDb = this.getWritableDatabase();
        ...
        long insert = harryPotterDb.insert(TABLE_QUESTION, null, values);
        return insert;
    }
     ...
     * Question
     */
    public List<HarryPotterQuestion> getAllQuestionsListHarryPotter() {
        List<HarryPotterQuestion> questionArrayList = new ArrayList<>();
        String selectQuery = "SELECT  * FROM " + TABLE_QUESTION;

        SQLiteDatabase harryPotterDb = this.getReadableDatabase();
        Cursor c = harryPotterDb.rawQuery(selectQuery, null);

        // looping through all records and adding to the list
        if (c.moveToFirst()) {
            do {
                HarryPotterQuestion question = new HarryPotterQuestion();
